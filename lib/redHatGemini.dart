...
class _RedHatState extends State<RedHat> {
  ...
  // A função de tradução para inglês foi removida.
  ...
  Future<void> _speakCurrentPage() async {
    ...
    try {
      print('Gerando áudio Google Cloud TTS em Português...');

      final response = await http.post(
        Uri.parse('$ttsEndpoint?key=$gemini'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'input': {'text': originalText},
          'voice': {
            'languageCode': 'pt-BR', // Português do Brasil
            'name': 'pt-BR-Wavenet-B', // Voz WaveNet de alta qualidade
          },
      ...
      if (response.statusCode == 200) {
        ...
        // O áudio vem codificado em Base64, é preciso decodificar.
        final String audioContentBase64 = data['audioContent'];
        final audioBytes = base64Decode(audioContentBase64);

        final tempDir = await getTemporaryDirectory();
        final tempFile = File('${tempDir.path}/gemini_narration.mp3');
