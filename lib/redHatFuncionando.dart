...
final List<StoryPage> chapeuzinhoVermelhoStory = [
  ...
  StoryPage(
    pageNumber: 2,
    text:
        'Um dia, sua mãe a chamou: "Chapeuzinho, leve esta cesta com bolo e vinho para a Vovó, que está doente. Vá direto e não saia do caminho!"',
    imageDescription:
...
class _RedHatState extends State<RedHat> {
  ...
  Future<void> _initTts() async {
    ...
    await flutterTts.setSpeechRate(0.3); // Lento e carinhoso
    await flutterTts.setVolume(1.0);
    await flutterTts.setPitch(1.45); // Voz mais doce e infantil

    if (Platform.isIOS) {
      // iOS: voz premium da Apple (muito natural)
      await flutterTts.setVoice({
        "name": "com.apple.voice.premium.pt-BR.Luciana",
        "locale": "pt-BR",
      });
    } else {
      // Android: seleciona a melhor voz neural disponível (2025)
      final voices = await flutterTts.getVoices;
      final preferredVoices = [
        "pt-BR-Neural2-B", // MELHOR VOZ FEMININA
        "pt-BR-Neural2-A",
        "pt-BR-Wavenet-B",
        "pt-BR-Wavenet-C",
        "pt-BR-Wavenet-A",
      ...
      for (String voiceName in preferredVoices) {
        ...
        if (voice.isNotEmpty) {
          ...
          print("Voz selecionada: $voiceName");
          break;
        }
      }
    }
    ...
    flutterTts.setErrorHandler((msg) {
      if (mounted) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text("Erro na narração: $msg")));
      }
    });
  }
  ...
  Future<void> _speakCurrentPage() async {
    ...
    final text = chapeuzinhoVermelhoStory[_currentPage].text;
